package com.example.commonlibrary.helper;
...
public class OptionsWindowHelper {
    ...
    public static CharacterPickerWindow builder(Activity activity, final OnOptionsSelectListener listener,Map<String, Map<String, List<String>>> map) {
        //选项选择器
        CharacterPickerWindow mOptions = new CharacterPickerWindow(activity);
        //初始化选项数据
        setPickerData(activity,mOptions.getPickerView(),map);
        //设置默认选中的三级项目
    ...
    public static void setPickerData(Activity activity,CharacterPickerView view,List<String> l1,List<List<String>> l2,List<List<List<String>>> l3){
        activity.runOnUiThread(() -> {
            //三级联动效果
            view.setPicker(l1, l2, l3);
        });
    }
    /**
     * 初始化选项数据
     */
    public static void setPickerData(Activity activity,CharacterPickerView view,Map<String, Map<String, List<String>>> mapMap) {
        options1Items = new ArrayList<>();
        options2Items = new ArrayList<>();
        options3Items = new ArrayList<>();

        final Map<String, Map<String, List<String>>> allCitys=mapMap;
